...
export class UsersService {
    ...
    async findUser(dto: FindUserDto) {
        let query = this.usersRepository
            ...
            .leftJoinAndSelect("users.institute", "institute")
        ...
        // searchTxt
        if (dto.searchTxt) {
            dto.searchTxt = dto.searchTxt.toLowerCase();

            query = query.andWhere(
                `
        (LOWER(users.fullname) like :fullname 
         or LOWER(users.email) like :email
         or LOWER(users.education_group) like :education_group
         or LOWER(users.phone) like :phone)
         `,
                {
                    fullname: `%${dto.searchTxt}%`,
                    email: `%${dto.searchTxt}%`,
                    education_group: `%${dto.searchTxt}%`,
                    phone: `%${dto.searchTxt}%`,
                    // birthdate: `${searchTxt}`,
                },
            );
        ...
        // institutes
        dto.institutes
            ? query.andWhere('institute.id in (:...institutes)', {
                institutes: dto.institutes,
            })
            : query;

        // states
        dto.states
            ? query.andWhere('state.id in (:...states)', {
                states: dto.states,
            })
            : query;
        ...
        // health_groups
        dto.health_groups
            ? query.andWhere('health_group.id in (:...health_groups)', {
                health_groups: dto.health_groups,
            })
            : query;
